...
class UMAFold(nn.Module):
    ...
    def forward(self, batch: dict) -> dict:
        ...
        """
        # Diagnostic: log which feature keys arrive with non-finite values. All
        # 1051 grad-guard fires from the prior run zeroed the exact same 105
        # params inside input_embedder.atom_attention_encoder (+ s_init),
        # suggesting an occasional NaN/Inf in the upstream feature dict rather
        # than generic bf16 drift. Toggled via `training.log_feature_nans`.
        if getattr(self, "_log_feature_nans", False):
            for k, v in batch.items():
                if torch.is_tensor(v) and v.is_floating_point() and not torch.isfinite(v).all():
                    print(
                        f"[feature_nan] key='{k}' shape={tuple(v.shape)} dtype={v.dtype}",
