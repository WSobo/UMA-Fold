# ==============================================================================
# UMA-Fold Makefile
# ==============================================================================
# All GPU commands run through SLURM srun or sbatch automatically.
# Never run `python script.py` directly — this is an HPC cluster.
#
# Usage:
#   make help         → list all targets
#   make pilot        → run 1-batch sanity check on GPU
#   make train        → submit full curriculum training job
#   make download     → submit dataset download job
#   make inference    → run inference on examples/target.yaml
#   make test         → run pytest suite (CPU, no GPU needed)
#   make lint         → run ruff linter
# ==============================================================================

# ── Environment ────────────────────────────────────────────────────────────────
VENV_ACTIVATE := source $(shell pwd)/.venv/bin/activate

# ── SLURM: Interactive GPU allocation (for srun one-off commands) ──────────────
SRUN_GPU := srun \
	--partition=gpu \
	--gres=gpu:A5500:1 \
	--mem=32G \
	--cpus-per-task=8 \
	--time=01:00:00

# ── SLURM: CPU-only allocation (for lightweight jobs like pytest/lint) ─────────
SRUN_CPU := srun \
	--partition=medium \
	--mem=8G \
	--cpus-per-task=4 \
	--time=00:30:00

# ── Project paths ──────────────────────────────────────────────────────────────
CONFIG      := configs/config.yaml
CKPT        := checkpoints/last.ckpt
INFER_YAML  := boltz/examples/multimer.yaml


# ==============================================================================
# HELP (default target)
# ==============================================================================
.PHONY: help
help:
	@echo ""
	@echo "  UMA-Fold Makefile"
	@echo "  ─────────────────────────────────────────────────────────"
	@echo "  make download       Submit SLURM dataset download job"
	@echo "  make preflight      CPU pre-flight check (login node, no queue)"
	@echo "  make pilot          Run 1-batch sanity check on GPU (srun)"
	@echo "  make pilot-15       Pilot with max_neighborhood=15"
	@echo "  make pilot-30       Pilot with max_neighborhood=30"
	@echo "  make pilot-40       Pilot with max_neighborhood=40"
	@echo "  make train          Submit full curriculum training, 1 GPU (sbatch)"
	@echo "  make train-multi    Submit full curriculum training, 4× GPU (sbatch)"
	@echo "  make check-multi    Pre-flight GPU check for DDP (10-min job, no curriculum)"
	@echo "  make inference      Run inference on $(INFER_YAML) (srun)"
	@echo "  make test           Run pytest suite (srun, CPU)"
	@echo "  make lint           Run ruff linter (srun, CPU)"
	@echo "  make env-setup      Create .venv via uv sync (run once after clone)"
	@echo "  make env-check      Verify .venv + torch/CUDA (srun GPU)"
	@echo "  ─────────────────────────────────────────────────────────"
	@echo ""


# ==============================================================================
# ENV SETUP
# ==============================================================================
# Run once after cloning: creates .venv with all dependencies via uv
# Requires uv to be installed: curl -LsSf https://astral.sh/uv/install.sh | sh
.PHONY: env-setup
env-setup:
	uv sync --extra dev


# ==============================================================================
# DATA
# ==============================================================================
.PHONY: download
download:
	sbatch scripts/SLURM/01_download_data.sh


# ==============================================================================
# PILOT RUN (sanity check — 1 batch, fast_dev_run=True)
# ==============================================================================
.PHONY: pilot
pilot:
	$(SRUN_GPU) bash -c '$(VENV_ACTIVATE) && \
		python -c "import torch; torch.set_float32_matmul_precision(\"high\")" && \
		python scripts/pilot_run.py'

.PHONY: pilot-15
pilot-15:
	$(SRUN_GPU) bash -c '$(VENV_ACTIVATE) && \
		python scripts/pilot_run.py \
		++data.datasets.0.cropper.max_neighborhood=15'

.PHONY: pilot-30
pilot-30:
	$(SRUN_GPU) bash -c '$(VENV_ACTIVATE) && \
		python scripts/pilot_run.py \
		++data.datasets.0.cropper.max_neighborhood=30'

.PHONY: pilot-40
pilot-40:
	$(SRUN_GPU) bash -c '$(VENV_ACTIVATE) && \
		python scripts/pilot_run.py \
		++data.datasets.0.cropper.max_neighborhood=40'

# Run all three pilot stages back-to-back in one srun allocation
.PHONY: pilot-all
pilot-all:
	sbatch scripts/SLURM/02_pilot_run.sh


# ==============================================================================
# TRAINING
# ==============================================================================
.PHONY: train
train:
	sbatch scripts/SLURM/03_train_model.sh

# Multi-GPU curriculum (4× A5500). Edit 03_train_multi_gpu.sh to change GPU count.
.PHONY: train-multi
train-multi:
	sbatch scripts/SLURM/03_train_multi_gpu.sh

# CPU pre-flight: imports, model init, config, data paths. Runs on login node instantly.
.PHONY: preflight
preflight:
	bash -c '$(VENV_ACTIVATE) && python scripts/preflight.py'

# Pre-flight GPU check for DDP: 1 batch with fast_dev_run on GPU (10-min job).
# Run before train-multi to catch runtime errors without burning queue time.
.PHONY: check-multi
check-multi:
	sbatch scripts/SLURM/check_ddp.sh


# ==============================================================================
# INFERENCE
# ==============================================================================
.PHONY: inference
inference:
	$(SRUN_GPU) bash -c '$(VENV_ACTIVATE) && \
		python -c "import torch; torch.set_float32_matmul_precision(\"high\")" && \
		python scripts/inference.py \
		--yaml $(INFER_YAML) \
		--config $(CONFIG) \
		--ckpt $(CKPT)'

# Inference with a custom yaml: make infer-yaml YAML=path/to/target.yaml
.PHONY: infer-yaml
infer-yaml:
	$(SRUN_GPU) bash -c '$(VENV_ACTIVATE) && \
		python scripts/inference.py \
		--yaml $(YAML) \
		--config $(CONFIG) \
		--ckpt $(CKPT)'


# ==============================================================================
# TESTING & LINTING (CPU only — no GPU needed)
# ==============================================================================
.PHONY: test
test:
	$(SRUN_CPU) bash -c '$(VENV_ACTIVATE) && \
		python -m pytest tests/ -v --tb=short'

.PHONY: test-fast
test-fast:
	$(SRUN_CPU) bash -c '$(VENV_ACTIVATE) && \
		python -m pytest tests/ -v --tb=short -x'

.PHONY: lint
lint:
	$(SRUN_CPU) bash -c '$(VENV_ACTIVATE) && ruff check src/ scripts/ tests/'

.PHONY: lint-fix
lint-fix:
	$(SRUN_CPU) bash -c '$(VENV_ACTIVATE) && ruff check --fix src/ scripts/ tests/'


# ==============================================================================
# ENV UTILITIES
# ==============================================================================

# Verify your GPU, CUDA, and torch setup are all wired correctly
.PHONY: env-check
env-check:
	$(SRUN_GPU) bash -c '$(VENV_ACTIVATE) && python -c "\
import torch; \
print(\"torch:\", torch.__version__); \
print(\"CUDA available:\", torch.cuda.is_available()); \
print(\"Device:\", torch.cuda.get_device_name(0) if torch.cuda.is_available() else \"CPU\"); \
print(\"bf16 supported:\", torch.cuda.is_bf16_supported() if torch.cuda.is_available() else False); \
print(\"venv active: .venv\")"'

# List jobs you currently have queued/running
.PHONY: jobs
jobs:
	squeue -u $$USER